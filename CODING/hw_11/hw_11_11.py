...
class Iterable:
    def __init__(self, max_vectors, max_points):
        ...
        self.vectors = []  # пустой список векторов
        self.max_vectors = max_vectors
        self.max_points = max_points

    def __next__(self):
        if self.current_index < self.max_vectors:  # создаем итератор до self.max_vectors
            # print('---->', self.current_index, '<----')
            self.vectors.append(Vector(Point(randrange(0, self.max_points),
                                             randrange(0, self.max_points))))  # добавляем в список новый вектор
            self.current_index += 1  # проходим дальше по итератору
            # возвращаем созданный вектор по индексу
            return self.vectors[self.current_index - 1]
        raise StopIteration  # выходим из итерации
...
# создаем итератор и итерируемся
